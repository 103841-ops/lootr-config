...
public class LootrConfConfig {
    ...
    static {
        ...
        builder.comment("Randomize Settings").push("randomize");

        RANDOMIZE_ENABLED = builder
                .comment("Enable randomizing all chest loot tables on chunk load")
                .define("enabled", false);

        RANDOMIZE_TABLES = builder
                .comment("List of custom loot table IDs to randomly assign (e.g. 'lootrconf:chests/common_loot')")
                .defineListAllowEmpty("tables", new ArrayList<>(), o -> o instanceof String);

        RANDOMIZE_WEIGHTS = builder
                .comment("Weights corresponding to each table (same order). Higher = more likely.")
                .defineListAllowEmpty("weights", new ArrayList<>(), o -> o instanceof Integer);

        RANDOMIZE_RADIUS = builder
                .comment("Radius around spawn to scan for chests (0 = no limit)")
